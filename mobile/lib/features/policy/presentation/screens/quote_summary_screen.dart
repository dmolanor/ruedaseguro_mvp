...
class QuoteSummaryScreen extends ConsumerStatefulWidget {
  const QuoteSummaryScreen({super.key, this.plan});
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Vehicle info
            _SectionTitle(title: 'Vehículo a asegurar')
            ...
            // Holder info
            _SectionTitle(title: 'Titular de la póliza')
            ...
            // Coverage SA table
            _SectionTitle(title: 'Tabla de coberturas')
                .animate(delay: 300.ms)
                .fadeIn(duration: 400.ms),
            const SizedBox(height: RSSpacing.sm),
            _CoverageSATable(plan: _plan)
            ...
            // Price breakdown
            _SectionTitle(title: 'Desglose de pago')
            ...
            // CTA
            RSButton(
              label: 'Solicitar emisión',
              onPressed: () =>
                  context.push('/payment/method', extra: _plan),
            )
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            plan.accentColor,
            plan.accentColor.withValues(alpha: 0.75)
          ],
    ...
    return RSCard(
      child: Column(
        children: [
          ...
          const Divider(height: RSSpacing.lg),
          _InfoRow(label: 'Aseguradora', value: MockPolicy.carrier),
          const SizedBox(height: RSSpacing.md),
          Container(
            width: double.infinity,
...
class _UpsellBanner extends StatelessWidget {
  const _UpsellBanner(
      {required this.upsellPlan, required this.currentPlan});
  final InsurancePlan upsellPlan;
  final InsurancePlan currentPlan;

  @override
  Widget build(BuildContext context) {
    final extraItems = upsellPlan.coverageItems.length -
        currentPlan.coverageItems.where((c) => c.sa != 'No incluido').length;
    return GestureDetector(
      onTap: () =>
          context.push('/policy/quote', extra: upsellPlan),
      child: Container(
        padding: const EdgeInsets.all(RSSpacing.md),
        decoration: BoxDecoration(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(upsellPlan.icon,
                  color: upsellPlan.accentColor, size: 20),
            ),
            const SizedBox(width: RSSpacing.md),
            Expanded(
              child: Column(
    ...
    return Row(
      ...
      children: [
        ...
        Text(
          ...
          style: isMono
              ? RSTypography.mono
                  .copyWith(fontSize: 14, color: RSColors.textPrimary)
