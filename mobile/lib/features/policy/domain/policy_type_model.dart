...
class PolicyTypeModel {
  ...
  factory PolicyTypeModel.fromMap(Map<String, dynamic> map) {
    return PolicyTypeModel(
      ...
      coverageDetails:
          (map['coverage_details'] as Map<String, dynamic>?) ?? {},
  ...
  List<String> get _coverageStrings {
    ...
    if (d['danos_cosas'] != null) {
      items.add('Daños a cosas de terceros hasta ${_usd(d['danos_cosas'] as num)}');
    }
    if (d['danos_personas'] != null) {
      items.add('Daños a personas de terceros hasta ${_usd(d['danos_personas'] as num)}');
    }
  ...
  List<CoverageItem> get _coverageItems {
    final d = coverageDetails;
    return [
      CoverageItem('Daños a cosas de terceros',
          d['danos_cosas'] != null ? _usd(d['danos_cosas'] as num) : 'No incluido'),
      CoverageItem('Daños a personas de terceros',
          d['danos_personas'] != null ? _usd(d['danos_personas'] as num) : 'No incluido'),
      CoverageItem('Asistencia legal',
          d['defensa_legal'] == true ? 'Incluida' : 'No incluido'),
      CoverageItem('Grúa 24/7', d['grua'] == true ? 'Incluida' : 'No incluido'),
      CoverageItem('Asistencia médica (Venemergencia)',
          d['asistencia_medica'] == true ? 'Incluida' : 'No incluido'),
      if (tier == 'plus' || tier == 'ampliada')
        CoverageItem('Accidentes personales 24/7',
